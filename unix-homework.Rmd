...
```{bash}
echo "my name is Brenna"
```

# print the absolute path of the top level of the homework directory where this is

```{bash}
echo "C:\Users\Brenna\Desktop\Bioinformatics\2-assignment2\unix-play-with-genomic-data-BrennaF"
cd "C:\Users\Brenna\Desktop\Bioinformatics\2-assignment2\unix-play-with-genomic-data-BrennaF"
```
...
```
drwxr-xr-x 1 Brenna None      0 Jan 29 10:16 .
drwxr-xr-x 1 Brenna None      0 Jan 28 21:21 ..
-rw-r--r-- 1 Brenna None      0 Jan 29 10:16 .Rhistory
drwxr-xr-x 1 Brenna None      0 Jan 28 21:21 .Rproj.user
drwxr-xr-x 1 Brenna None      0 Jan 29 10:16 .git
-rw-r--r-- 1 Brenna None    134 Jan 28 21:21 .gitignore
```
...
```
I got down to 12 (counting TABs and typing "ls")
```
...
```
To be sure you're removing the things you think you are!
```

# If you had just typed `rm -r bad directory name with spaces` what would Unix have tried to remove?

```
It would have looked for directories named "bad", "directory", "name", "with", and "spaces".
```
...
```{bash}
zcat ./fastq/DPCh_plate1_C11_S35.R1.fq.gz | head -n 8  # gzcat not available, used zcat instead
```
...
```
Differ only on the last character - which signifies Read 1 or Read 2
```
...
```{bash}
sha1sum R1_all_via_cat.fq R1_all_via_zcat.fq  # use sha1sum on windows
```
...
```{bash}
tail ./sam/DPCh_plate1_A05_S5.sam -n 15
```
...
```{bash}
head ./sam/DPCh_plate1_A05_S5.sam -n 15
```
...
```{bash}
./count_files.sh  # worked for me the first time!

chmod 664 count_files.sh  # chmod doesn't seem to be working correctly on windows??
```
